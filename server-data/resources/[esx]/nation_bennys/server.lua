...
ESX.RegisterServerCallback('nation:checkPayment',function(source, cb, amount)
    ...
    if not config.mechaniconly and bankey >= amount then
        ...
        TriggerClientEvent("Notify",source,"sucesso","Upgrade <b>Succeess Faggot</b><br>Costo totale <b>$"..tonumber(amount).." $ <b>.",7000)
        cb(true)
    elseif not config.mechaniconly and cash >= amount then
        ...
        TriggerClientEvent("Notify",source,"sucesso","Upgrade <b>Succeess</b><br>Costo Totale <b>$"..tonumber(amount).." $ <b>.",7000)
        cb(true)
    elseif config.mechaniconly and bankey >= amount or config.mechaniconly and cash >= amount then
        if config.societymoney then
            ...
            elseif cash >= amount then
                xPlayer.removeMoney(amount)
                TriggerClientEvent("Notify",source,"sucesso","Upgrade <b>Succeess</b><br>osto totale <b>$"..tonumber(amount).." $ <b>.",7000)
                cb(true)
            end
        end
    else
